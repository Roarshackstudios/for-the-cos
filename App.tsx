...
const App: React.FC = () => {
  ...
  // Fix: Corrected typo in state declaration where useState call was missing
  ...
  // Allow admin gear if either not configured (to set up) or if logged in as admin
  const isAdmin = (state.currentUser?.email?.toLowerCase() === ADMIN_EMAIL_LOWER) || !isSupabaseConfigured || (!state.currentUser && isSupabaseConfigured);
  ...
  const nexusSchemaFix = `-- NEXUS SCHEMA FIX --
...
-- 2. Add Missing Columns to Profiles (if table exists but is partial)
...
-- 4. Add Missing Columns to Generations
...
-- 7. RLS Configuration (Enable and grant public access for development)
ALTER TABLE public.profiles ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.generations ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.likes ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.orders ENABLE ROW LEVEL SECURITY;

CREATE POLICY IF NOT EXISTS "Public Access" ON public.profiles FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY IF NOT EXISTS "Public Access" ON public.generations FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY IF NOT EXISTS "Public Access" ON public.likes FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY IF NOT EXISTS "Public Access" ON public.orders FOR ALL USING (true) WITH CHECK (true);`;
            ...
            <div className="flex-grow overflow-y-auto custom-scrollbar space-y-10 pr-4">
               ...
               <div className="p-10 bg-red-600/10 border border-red-500/30 rounded-[2.5rem] space-y-8">
                  <div className="flex items-center space-x-3">
                    ...
                    <p className="text-lg font-black text-red-500 uppercase tracking-widest">NEXUS SCHEMA FIX (SQL)</p>
                  </div>
                  <div className="p-6 bg-red-500/5 rounded-2xl border border-red-500/10">
                     <p className="text-[11px] text-zinc-300 font-black leading-relaxed uppercase mb-2">RUN THIS SCRIPT IN SUPABASE SQL EDITOR TO FIX MISSING COLUMNS:</p>
                  </div>
                  <div className="relative group">
                    <pre className="w-full bg-black border border-zinc-800 rounded-2xl p-6 text-[10px] font-mono text-blue-400 overflow-x-auto h-64 custom-scrollbar select-all">
                      {nexusSchemaFix}
                    </pre>
                    <button 
                      onClick={() => { navigator.clipboard.writeText(nexusSchemaFix); alert("SQL script copied to clipboard!"); }}
                      className="absolute top-4 right-4 bg-blue-600 hover:bg-blue-500 text-white px-6 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all shadow-xl"
                    >
                      COPY SCRIPT
                    </button>
                  </div>
                  <p className="text-[9px] text-zinc-500 font-black uppercase tracking-widest bg-zinc-900 p-4 rounded-xl text-center">After running the script, refresh this app to sync neural cache.</p>
               </div>

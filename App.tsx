...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    checkAiKey();
    if (isSupabaseConfigured) loadGallery();
  ...
  const saveToHistory = async () => {
    ...
    if (!isSupabaseConfigured) {
      alert("Cloud Chronicles not connected. Please enter your Supabase URL and Key in the Matrix Config (Gear icon) to archive your masterpieces.");
      setShowAdmin(true);
      return;
    }
    ...
    try {
      ...
      if (targetRef.current) {
        ...
        await loadGallery();
        alert("Artifact archived in Cloud Chronicles.");
      }
    } catch (err: any) {
  ...
  const handlePurchase = async (itemType: 'comic' | 'card') => {
    ...
    if (confirm(`Authorize purchase of ${itemName} for $${price}?`)) {
      ...
      try {
        ...
        alert("Artifact production queued in Cloud Operations.");
      } catch (e: any) {
        alert("Transaction recorded locally, but cloud sync failed.");
      }
    }
  };
  ...
  const resetSupabase = () => {
    if (confirm("Reset cloud credentials? This will force a page reload.")) {
      setAdminSettings(prev => ({ ...prev, supabaseUrl: "", supabaseAnonKey: "" }));
      localStorage.removeItem('cos-admin-settings'); 
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white flex flex-col font-inter">
        ...
        <div className="flex items-center space-x-6">
          <div className="flex flex-col items-end mr-4">
            <div className="flex items-center space-x-2">
              <div className={`w-2 h-2 rounded-full ${isSupabaseConfigured ? 'bg-green-500 animate-pulse' : 'bg-red-500 animate-pulse'}`}></div>
              <span className="text-[9px] font-black uppercase tracking-widest text-zinc-500">Sync: {isSupabaseConfigured ? 'Cloud Mode' : 'Disconnected'}</span>
            </div>
            <div className="flex items-center space-x-2">
        ...
        <div className="fixed inset-0 z-[100] bg-black/90 flex items-center justify-center p-6 backdrop-blur-md animate-fade-in">
          <div className="max-w-2xl w-full bg-zinc-900 border border-zinc-800 rounded-[2.5rem] p-10 space-y-8 shadow-2xl relative overflow-hidden">
            ...
            <div className="space-y-6 max-h-[60vh] overflow-y-auto px-2 custom-scrollbar">
              ...
              <div className="p-6 bg-blue-600/10 border border-blue-500/30 rounded-3xl space-y-4 relative overflow-hidden">
                <div className="flex items-center justify-between">
                  ...
                  <div className={`px-2 py-1 rounded text-[8px] font-bold uppercase ${isSupabaseConfigured ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                ...
                <div className="text-[10px] text-zinc-400 leading-relaxed bg-black/30 p-4 rounded-xl space-y-2">
                  <p className="font-bold text-white uppercase italic tracking-wider">Configure Cloud Storage to archive masterpieces:</p>
                  <ol className="list-decimal list-inside space-y-1 ml-1">
                    ...
                    <li>Copy <b>Project URL</b> and <b>anon public</b> API key</li>
                  </ol>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-start items-center pt-2">
                    <button onClick={resetSupabase} className="text-[9px] font-black uppercase text-red-500/50 hover:text-red-500 transition-colors">Clear Cloud Credentials</button>
                  </div>
                </div>
              </div>
            ...
            <button onClick={() => {
              setShowAdmin(false);
              window.location.reload(); 
            }} className="w-full py-4 bg-blue-600 text-white rounded-2xl font-black uppercase tracking-widest hover:bg-blue-500 transition-all shadow-xl shadow-blue-500/20">Sync & Initialize Cloud</button>
          </div>
        </div>
      )}
      ...
      <main className="flex-grow flex flex-col justify-center">
        ...
        {state.step === AppStep.RESULT && state.resultImage && (
          <div className="p-6 max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-12 gap-12 animate-fade-in">
            ...
            <div className="absolute left-[-9999px] top-0 overflow-hidden">
              <div ref={rawImageRef} style={{ width: '800px', height: '1000px' }}><img src={state.resultImage} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="Export Raw" /></div>
              <div ref={comicExportRef} style={{ width: '800px', height: '1200px', position: 'relative' }}>
                <img src={state.resultImage} style={{ width: '100%', height: '100%', objectFit: 'cover', transform: `scale(${state.resultScale}) translate(${state.resultOffset.x}%, ${state.resultOffset.y}%)` }} alt="Export Comic" />
              ...
              <div className={`relative w-full max-w-md bg-black rounded-3xl overflow-hidden border-[8px] border-zinc-900 shadow-2xl transition-all duration-500 ${state.isComicStyled ? 'aspect-[2/3]' : 'aspect-[3/4]'}`}>
                {state.isComicStyled ? (
                  <div className="w-full h-full relative">
                    <img src={state.resultImage} className="w-full h-full object-cover" style={{ transform: `scale(${state.resultScale}) translate(${state.resultOffset.x}%, ${state.resultOffset.y}%)` }} alt="Comic Mode Preview" />
                    <ComicFrame category={state.selectedCategory?.name || "HERO"} subcategory={state.selectedSubcategory?.name || "ULTIMATE"} customTitle={state.characterName} />
                  </div>
                ) : state.isCardStyled ? (
                  ...
                  <img src={state.resultImage} className="w-full h-full object-cover" alt="Processed Cosplay" />
                )}
              </div>
            ...
            <div className="lg:col-span-5 xl:col-span-4 space-y-8">
              <div className="bg-zinc-900/50 p-8 rounded-[2.5rem] border border-zinc-800 space-y-8 backdrop-blur-xl">
                ...
                <button onClick={saveToHistory} disabled={isSaving} className="w-full py-5 bg-white text-black rounded-full font-black uppercase tracking-widest flex items-center justify-center space-x-3 hover:bg-blue-500 hover:text-white transition-all shadow-xl disabled:opacity-50 active:scale-95">
                  ...
                  <span>{isSaving ? 'Synchronizing Cloud...' : 'Archive in Chronicles'}</span>
                </button>
              </div>
        ...
        {state.step === AppStep.GALLERY && (
          <div className="p-8 max-w-7xl mx-auto w-full space-y-12 animate-fade-in">
             ...
             {!isSupabaseConfigured ? (
               <div className="flex flex-col items-center justify-center py-40 space-y-6 opacity-40">
                 <i className="fa-solid fa-cloud-slash text-7xl"></i>
                 <p className="font-orbitron font-bold uppercase tracking-[0.5em] text-sm text-center">Cloud Synchronization Required<br/><span className="text-[10px] font-mono tracking-normal text-zinc-500">Configure Supabase in the Matrix Config to view archived artifacts.</span></p>
                 <button onClick={() => setShowAdmin(true)} className="px-6 py-2 border border-zinc-700 rounded-full text-[10px] font-black uppercase tracking-widest hover:bg-white hover:text-black transition-all">Setup Cloud</button>
               </div>
               ...
               <div className="flex flex-col items-center justify-center py-40 space-y-6 opacity-20">
                 ...
                 <p className="font-orbitron font-bold uppercase tracking-[0.5em] text-sm">No records archived in the cloud</p>
               </div>
             ) : (
      ...
      <footer className="py-12 text-center border-t border-zinc-900 text-zinc-700 text-[10px] font-bold uppercase tracking-[0.4em] bg-black/80 backdrop-blur-md">
        © 2024 FOR THE COS. DIMENSIONS SYNERGIZED VIA CLOUD NEURAL PROCESSING.
      </footer>
    </div>
  );
};
